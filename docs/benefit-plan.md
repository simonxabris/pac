...
removal operation unless passed `--allow-delete`. `pac plan` always renders them.
...
- deploy asks for consent before destructive operations unless `--allow-delete` is passed.
