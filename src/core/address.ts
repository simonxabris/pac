...
export const ResourceKey = Schema.String.check(
  Schema.makeFilter((value) => resourceKeyPattern.test(value), {
    message: "Resource keys must match [a-zA-Z][a-zA-Z0-9_-]*",
  }),
);

export const ResourceAddress = Schema.String.check(
  Schema.makeFilter((value) => resourceAddressPattern.test(value), {
...
export const makeAddress = <const Kind extends ResourceKind>(
  kind: Kind,
  key: string,
): ResourceAddress<Kind> =>
  decodeResourceAddress(
    `${decodeResourceKey(kind)}.${decodeResourceKey(key)}`,
  ) as ResourceAddress<Kind>;
