...
const productUpdatePayload = (change: ResourceChange, managed: ProductManagedV1): JsonObject => {
  ...
  if (hasDiff(change, "/prices")) entries.push(["prices", managed.prices.map(pricePayload)]);
  return decodeJsonObject(Object.fromEntries(entries));
};

export const planProductCreate = (resource: CanonicalResource): Effect.Effect<ReadonlyArray<Operation>> =>
  Effect.succeed([
    {
    ...
    return [
      {
        ...
        preview: {
          ...
          lines: change.diffs.map((diff) => `${diff.path}: ${JSON.stringify(diff.before)} -> ${JSON.stringify(diff.after)}`),
        },
      },
    ];
  });

export const planProductArchive = (resource: CanonicalResource): Effect.Effect<ReadonlyArray<Operation>, ReturnType<typeof errorDiagnostic>> =>
  Effect.gen(function*() {
